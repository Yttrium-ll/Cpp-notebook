...
// int dummy::count = 0;		// инициализаци€ статического члена класса (если объ€вление без слова inline)

dummy::dummy() : x{ 0 }, y{ 0 }, name{ "dummy" }		// список инициализации: инициализаци€ членов вне тела конструктора
{
	num = count++;
...
void dummy::takeAction()
{
	...
	if (action) action();		// вызов std::function производитс€ как обычно (вызовет исключение std::bad_function_call, если будет пустым)
}
